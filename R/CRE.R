...
#' @param mode There are two modes to calculate CRE: "classic" and "multidimension" (experimental). "classic" computes the monodimension CRE on a single matrix. "multidimension" requires a list of matrices and calculates the multidimensional cumulative residual probability. Default is "classic".
#' @param rasterOut Boolean; if TRUE, the output will be in SpatRaster format with \code{x} as the template.
#' @param rescale Boolean; if TRUE, \code{x} will be scaled and centred to standardise different matrices if mode is "multidimension". Default is FALSE.
#' @param na.tolerance Numeric value between 0.0 and 1.0 indicating the proportion of NA values tolerated in each moving window over \code{x} for CRE calculation. If the proportion of NAs exceeds \code{na.tolerance}, the value for that window is set as NA. Default is 1.0 (full tolerance for NAs).
#' @param simplify Number of decimal places retained in the CRE calculation. Applies only if \code{x} is floats.
#' @param np Number of processes (cores) to be used. Default is 1.
#' @param cluster.type Specifies the type of cluster to create. Options are "MPI" (for "makeMPIcluster"), "FORK", and "SOCK" (for "makeCluster"). Default is "SOCK".
#' @param debugging Boolean; if TRUE, additional debug messages will be printed. Default is FALSE.
#'
#' @details
...
#'
#' @references
#' Rao M, Chen Y, Vemuri BC, Wang F (2004) Cumulative Residual Entropy: A New Measure of Information. IEEE Transactions on Information Theory 50:1220–1228.
#'
#' @author Saverio Vicario \email{saverio.vicario@cnr.it}
#'
#' @examples
#' \dontrun{
...
{
message("CRE has been temporarily removed from `rasterdiv`. Be in touch with the developer for more infos...")
}