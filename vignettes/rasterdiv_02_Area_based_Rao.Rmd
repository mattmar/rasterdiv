...
noise[] = as.integer(runif(n=ncell(noise),min=0, max=10))
RaoMC <- paRao(x=c(copNDVIlr,noise), area=world, field='CONTINENT', alpha=10, method="multidimension", simplify=0)
```

```{r fig02, warning=FALSE, message=FALSE}
#Plot area-based RAo's index
plot(RaoMC, col=hcl(RaoMC$alpha.10*10000), main="Multidimension Rao's index per continent alpha 10")
text(RaoMC, label=paste("Rao'Q =", round(RaoMC$alpha.10,3)), col="black", family="Arial", halo=TRUE)
```
...
```{r fig03, echo = T, results = 'hide', warning=FALSE, message=FALSE}
