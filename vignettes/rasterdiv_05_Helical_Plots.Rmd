...
```{r results='hide', message=FALSE, warning=FALSE}
# Prepare NDVI data
dataPrepNDVI <- heliPrep(dates, avgNDVI, window_size=14)
...
covidPrep <- lapply(unique(covidCases.df$administrative_area_level_1), function(x) {
  ...
  outPrep <- heliPrep(dates=temp$date, values=temp$dailyDeaths, window_size=3)
  outPrep$country=x
  return(outPrep)
  })
...
heliPlot can be customized with ggplot2 functions to match specific styles or references. For example, to mimic the visualizations from `r if(knitr::is_html_output()) {
...
This customization includes manual colour scales, legend adjustments, and panel border styling to closely replicate the desired appearance from the reference article.